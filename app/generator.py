...
def _load_model() -> None:
    ...
    # Set HF_TOKEN for gated model access (hf-mirror.com also requires it)
    if HF_TOKEN and "HF_TOKEN" not in os.environ:
        os.environ["HF_TOKEN"] = HF_TOKEN

    # Check and clear partial cache if model_config.json is missing
...
def generate_audio(prompt: str, duration_seconds: int) -> tuple[str, int]:
    ...
    try:
        ...
        # Save WAV
        sf.write(output_path, output.cpu().numpy().T, sample_rate)
